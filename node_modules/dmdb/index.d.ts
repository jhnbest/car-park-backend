...
declare namespace dmdb {
     ...
     * 是否自动提交
     * 自动提交配置优先级顺序从高到低如下：
     * （1）执行选项 ExecuteOptions.autoCommit / ExecuteManyOptions.autoCommit
     * （2）驱动全局配置 dmdb.autoCommit
     * （3）连接串属性/连接属性 localhost:5236?autoCommit=true / ConnectionAttributes.autoCommit
     * @default undefined
     */
    ...
    /**
     * 是否将结果集中数据库JSON/JSONB类型的数据解析成JSON对象返回
     * @default true
     */
    let parseJson: boolean;
    ...
    interface ConnectionAttributes {
        ...
        /**
         * 服务名连接策略
         * 0：依次选择不同节点，使连接均匀分布 在各个节点上
         * >0：选择列表中第n个节点，只有当前节点无法连接时才会选择下一个节点
         * @default 0
         */
        epSelector?: number;
        /**
         * 是否根据节点存活状态动态调整epSelector的值，
         * 若开启，节点故障并转移到下一个节点后，即使前面的节点故障恢复了，后续也连接不回前面的节点上
         * @default false
         */
        epSelectorDynamic?: boolean;
        /**
         * 是否处理转义字符
         * @default false
         */
        ...
        /**
         * 绑定参数为数组时，是否直接注入到SQL语句中，非数组类型的参数不受影响。
        ...
        isBdtaRS?: boolean;
        /**
         * 客户端语言 CN：简体中文，EN：英文，CNT_HK：繁体中文
         * @default 'CN'
         */
        language?: string;
        /**
        ...
        /**
         * 服务名方式连接数据库时只选择模式匹配的库进行连接，开启读写分离时本配置不生效
        ...
        /**
         * 是否自动分发，否则为利用连接上的readOnly属性分发
         * @default true
         */
        rwAutoDistribute?: boolean;
        /**
         * 是否开启读写分离系统高可用模式
         * @default false
         */
        ...
        rwPercent?: number;
        /**
         * 是否开启读写分离，当启用读写分离时，loginMode失效
         * 0 - 不启用
         * 1 - 启用
         * 2 - 启用，备库由客户端选择，且只会选择服务名中配置的节点
         * 3 - 启用，备库由服务器选择
         * 4 - 启用，备库由客户端选择，且只连接事务一致性备库
         * @default 0
         */
        rwSeparate?: number;
        /**
